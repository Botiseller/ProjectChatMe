...
{
    public abstract class HttpClientWrapperNoUserBase : IDisposable
    {
        private AsyncLazy<HttpClient> _lazyClient;

        public WebApiResult ResultState { get; set; }

        protected HttpClientWrapperNoUserBase()
        {
            ServicePointManager.Expect100Continue = false;
            ServicePointManager.CheckCertificateRevocationList = false;
            HttpClient client = HttpClientFactory.Create(new HttpClientMessageHpHandnler());
